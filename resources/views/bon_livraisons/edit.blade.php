...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-200 mt-8">
    ...
    <form action="{{ route('bon_livraisons.update', $bonLivraison->idBL) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="numeroBL" value="{{ old('numeroBL', $bonLivraison->numeroBL) }}" class="w-full border rounded px-3 py-2 @error('numeroBL') border-red-500 @enderror" required>
        ...
        <div>
            ...
            <select name="devi_id" class="w-full border rounded px-3 py-2 @error('devi_id') border-red-500 @enderror" required>
                ...
                @foreach($devis as $devi)
                    <option value="{{ $devi->id }}" {{ old('devi_id', $bonLivraison->devi_id) == $devi->id ? 'selected' : '' }}>{{ $devi->id }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="client_id" class="w-full border rounded px-3 py-2 @error('client_id') border-red-500 @enderror" required>
                ...
                @foreach($clients as $client)
                    <option value="{{ $client->id }}" {{ old('client_id', $bonLivraison->client_id) == $client->id ? 'selected' : '' }}>{{ $client->nom }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="statut" value="{{ old('statut', $bonLivraison->statut) }}" class="w-full border rounded px-3 py-2 @error('statut') border-red-500 @enderror">
